...
class opisController extends Controller {
  ...
  public function getDepartments() {
   // $result = DB::table("dipartimento")->get();
      $result = DB::select("SELECT * FROM dipartimento");
    // echo response()->json($result);
    return response()->json($result);
  }
    
    //questa resta invariata
  ...
  public function getCds($department = "") {
    ...
    // in questa getTeachings bisogna aggiungere l'anno accademico nella where. Supponendo di avere nel db diversi record riguardanti lo stesso insegnamento in relazione al canale ED ANNO ACCADEMICO, se nella where non specifico l'anno ( che non so come dovrà essere passato alla funzione tramite questo oggetto Request passato come parametro) il risultato sarà errato.
  
    public function getTeachings($cds) {
    if ($cds != "") {
      $result = DB::select("SELECT * FROM insegnamento WHERE id_cds = '".$cds."'");
      return response()->json($result);
    }
  }
